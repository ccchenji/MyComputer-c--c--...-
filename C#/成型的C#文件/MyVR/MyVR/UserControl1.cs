...
namespace MyVR
{
    public partial class UserControl1 : UserControl
    {
        ...
        private void DrawSmallRound(Graphics g)//画动态的小圆，以及指示
        {
            ...
            if (Math.Abs(SetAngle - ReallAngle) >= 1 && Math.Abs(SetAngle - ReallAngle) < 2)
            {
                g.DrawLine(new Pen(Color.SkyBlue, 2), (float)(this.Width / 2 + 25 * Math.Sin((SetAngle - ReallAngle + c) * 3.14 / 180)) + b, (float)(this.Height / 2 - 25 * Math.Cos((SetAngle - ReallAngle + c) * 3.14 / 180)) - a,
                    (float)(this.Width / 2 + 25 * Math.Sin((SetAngle - ReallAngle + c) * 3.14 / 180)) - b, (float)(this.Height / 2 - 25 * Math.Cos((SetAngle - ReallAngle + c) * 3.14 / 180)) + a);
            }
            if (Math.Abs(SetAngle - ReallAngle) >2)
            {
                g.DrawLine(new Pen(Color.SkyBlue, 2), (float)(this.Width / 2 + 55 * Math.Sin((SetAngle - ReallAngle + c) * 3.14 / 180)) + b, (float)(this.Height / 2 - 55 * Math.Cos((SetAngle - ReallAngle + c) * 3.14 / 180)) - a,
                    (float)(this.Width / 2 + 55 * Math.Sin((SetAngle - ReallAngle + c) * 3.14 / 180)) - b, (float)(this.Height / 2 - 55 * Math.Cos((SetAngle - ReallAngle + c) * 3.14 / 180)) + a);
            }
        }
