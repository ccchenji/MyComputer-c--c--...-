...
namespace 控制软件
{
    ...
    public partial class Form1 : Form
    {
        ...
        //用于将频率数值转化为控制字指令。转化有问题，用枚举，暂时不用此方式。(原变量定义冲突!)
        // private int freNum;
        // private string freNum1;
        // private string  freNum2;
        // private string freNum3;
        ...
        private void Form1_Load(object sender, EventArgs e)   //设置端口基本信息 vor表盘中预设航道角初始化 定时器
        {
            for (int i = 0; i < 380; i++)
            {
            ...
            comboBox1.Text = "45";
            SetAngle.Text = "0";//预设航道角初值化为0
            MyVor.setAngle = double.Parse(SetAngle.Text);//把预设航道角值送到vor表盘中
            //初始化下拉串口名称列表框
        ...
        private void openclose_Click(object sender, EventArgs e)        // 串口开关
        {
            ...
            SignalFre.Enabled = comm.IsOpen;    //端口关闭时灰掉频率设置键
            PhaseOffset.Enabled = comm.IsOpen;//端口关闭时灰掉方位角设置键
            SendBtn.Enabled = comm.IsOpen;//端口关闭时灰掉发送键
            button1.Enabled = comm.IsOpen;//端口关闭时灰掉性能测试键
        }
        ...
        byte[] shift_buf = new byte[32];//声明一个缓冲数组收到的命令字4字节32位
        int shift_buf_rptr = 0;
        byte[] fifo_buf = new byte[16];//声明一个缓冲数组收到的命令字2字节16位，与接受状态相关
        byte[] fifo_buf_bak = new byte[16];//声明一个缓冲数组收到的命令字2字节16位，与待发送指令相关
        int ptr_w = 0;
        Boolean StatasOkFlag = false;//状态可行初始化为否
        int StatusWordGotCnt = 0;//收到显示为0
        private void timer1_Tick_1(object sender, EventArgs e)         //定时器(1s)函数 筛选出待显示信息
        {
            ...
            if (StatasOkFlag)
            {
                StatasOkFlag = false;
                StatusWordGotCnt++;
                StatsWordGotNo.Text = "已收到：" + StatusWordGotCnt.ToString();//显示已收到多少个状态字
                int a0 = fifo_buf[0], a1 = fifo_buf[1], a2 = fifo_buf[2], a3 = fifo_buf[3];//4个字节分区。
                //B18：本地模式启动
                ...
                LocalModeEn = LocalModeChk.Checked;//当本地模式开启，以下按键全灰
                ...
                {
                    //B27:设置实采数据，默认0.
                    //  realdatatest.Text = Convert.ToString((a0 & 0xf7));
                    //B26~24: 设置音量
                ...
                try
                {
                    MyVor.reallAngel = Double.Parse(RecDegtextBox.Text);//vor表盘中的真实角度等于测量角度
                    if (TestForm.form2 != null) TestForm.form2.SendMessage();
                }
                catch
                {
        ...
        //远程控制模式下下，PC机可以设置ACARS电台的工作模式及参数
        //远程控制模式下下，PC机可以设置ACARS电台的工作模式及参数
        //远程控制模式下下，PC机可以设置ACARS电台的工作模式及参数

        int OrderSentCnt = 0;
        private void Send_Click_1(object sender, EventArgs e)//待发送指令发送按钮
        {
        ...
        //        case 0:  Send_text.Text = "10 01 B5 E4"; break;  //112.1 Tjanjin Airport VOR
        //        case 1:  Send_text.Text = "10 01 B6 AC"; break;  //113.4 东丽湖VOR
        //        case 2:  Send_text.Text = "10 01 B7 74 "; break;  //118.2 TJTWR
        //        case 3:  Send_text.Text = "10 01 B8 3C "; break; //119.0 BJAPP2
        //        case 4:  Send_text.Text = "10 01 B9 04 "; break; //125.6 BJAPP1
        //        case 5:  Send_text.Text = "10 01 B9 CC "; break; //126.4 ATIS
        //        case 6:  Send_text.Text = "10 01 BA 94"; break;  //127.9 TJAPP
        //        case 7:  Send_text.Text = "10 01 BB 5C"; break;  //131.45 ACARS
        //        case 8:  Send_text.Text = "10 01 BC 24"; break;  //1030MHz 二次雷达发射 
        //        case 9:  Send_text.Text = "10 01 BC EC"; break;  //1090MHz 二次雷达接收
        //        case 10: Send_text.Text = "10 01 BD B4"; break;
        //        case 11: Send_text.Text = "10 01 BE 7C"; break;
        //        case 12: Send_text.Text = "10 01 BF 44"; break;
        //        case 13: Send_text.Text = "10 01 C0 0C"; break;
        //        case 14: Send_text.Text = "10 01 C0 D4"; break;
        //        case 15: Send_text.Text = "10 01 C1 9C"; break;
        //        case 16: Send_text.Text = "10 01 C2 64"; break;
        //        case 17: Send_text.Text = "10 01 C3 2C"; break;
        //        case 18: Send_text.Text = "10 01 C3 F4"; break;
        //        case 19: Send_text.Text = "10 01 C4 BC"; break;
        //        case 20: Send_text.Text = "10 01 C5 84"; break;
        //        case 21: Send_text.Text = "10 01 C6 4C"; break;
        //        case 22: Send_text.Text = "10 01 C7 14"; break;
        //        case 23: Send_text.Text = "10 01 C7 DC"; break;
        //        case 24: Send_text.Text = "10 01 C8 A4"; break;
        //        case 25: Send_text.Text = "10 01 C9 6C"; break;
        //        case 26: Send_text.Text = "10 01 CA 34"; break;
        //        case 27: Send_text.Text = "10 01 CA FC"; break;
        //        case 28: Send_text.Text = "10 01 CB C4"; break;
        //        case 29: Send_text.Text = "10 01 CC 8C"; break;
        //        default: Send_text.Text = "10 02 01 7A "; break;//131.45 ACARS
        //            /*
        //            a =(int.Parse(SignalFre.Text))*1000;
        //    a1 =(a % 256).ToString("X2");//变量定义冲突!!
        //    a = a / 256;
        //    a2 = (a % 256).ToString("X2");
        //    a = a / 256;
        //    a3=(a%256).ToString("X2");
        //    Send_text.Text = "10 "+a3+" "+ a2+" " + a1;
        //            */
        //        //
        //    }
        //    // a.ToString("x8");
        ...
        //}
        ////B27:实采数据测试 
        private void realdatatest_CheckedChanged(object sender, EventArgs e)//定义实采数据测试按钮点下数值为1，方便发送给下位机识别
        {
            /*
            if (realdatatest.Checked)
                fifo_buf_bak[0] = (byte)(fifo_buf_bak[1] | 0x08);
            else
                fifo_buf_bak[0] = (byte)(fifo_buf_bak[1] & 0xf7);
            send_str(0);
            */
        }
        ...
        {
            int dat;
            dat = (int)(Convert.ToDecimal(PhaseOffset.Text));//根据飞机方位角转换发送命令
        ...
        {
            if (SetAngle.Text != "")//如果预设航道角方不为空，把角度值送到vor表中的设定角度去
            {
                try
                {
                    MyVor.setAngle = double.Parse(SetAngle.Text);
                    PlaneTrack.SetAngle = double.Parse(SetAngle.Text);//把预设航道角送到飞机航迹中的预设航道角去用来显示预选航道
                }
                catch
                {
                    return;
            ...
            try
            {
                if (PlaneTrack.Round == true && (double.Parse(PhaseOffset.Text) != PlaneTrack.ReallAngle))
                {
                    PhaseOffset.Text = PlaneTrack.ReallAngle.ToString();//如果选择圆形轨道，把实时角度值送给飞机方位角去
                    NoiseLev_SelectedIndexChanged(this, new EventArgs());//执行这个命令，把角度信息指令发给下位机
                }
            }
            catch
            {
        ...
        private void button1_Click(object sender, EventArgs e)//利用button1按键调用form2
        {
            TestForm.form2 = new TestForm();//在Form1将form2实例化
            TestForm.form2.Show();//显示form2.
            button1.Enabled = false;
        }

        private void MyVor_Load(object sender, EventArgs e)
        {

        }

        private void PlaneTrack_AutoSizeChanged(object sender, EventArgs e)
        {

        }

        private void IF_Amp_Box_KeyPress(object sender, KeyPressEventArgs e)
        {

        }

        private void Send_text_TextChanged(object sender, EventArgs e)//待发送指令文本框。若文本框内容改变则执行。
        {
            //端口关闭，不能发送数据.
        ...
        private void RecDegtextBox_TextChanged(object sender, EventArgs e)
        {

        }


        private void SignalFre_Leave(object sender, EventArgs e)
        {
            int a;
            string a1, a2, a3;
            a = (int)((double.Parse(SignalFre.Text)) * 1000);
            a1 = (a % 256).ToString("X2");//变量定义冲突!!
            a = a / 256;
            a2 = (a % 256).ToString("X2");
            a = a / 256;
        ...
        private void IF_Amp_Box_TextChanged(object sender, EventArgs e)
        {
            try
            {
                MyVor.IF_Amp = double.Parse(IF_Amp_Box.Text);
            }
            catch
            {
        ...
        private void comboBox1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                PlaneTrack.SinAngle = int.Parse(comboBox1.Text);
            }
            catch
            {
                return;
